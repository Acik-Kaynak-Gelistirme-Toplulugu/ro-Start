...
class DriversPage(QWidget):
    ...
    def check_system_status(self):
        ...
        self.session_label.setText(
            f"<small style='color: #a6adc8;'>{tr.t('drivers.session_type')}: {session_type.capitalize()}</small>"
        )
            ...
            if shutil.which("nvidia-smi"):
                ...
                if version:
                    self.version_label.setText(
                        f"{tr.t('drivers.driver_installed')} {version} <span style='color:#a6e3a1;'>({tr.t('drivers.driver_current')})</span>"
                    )
                else:
